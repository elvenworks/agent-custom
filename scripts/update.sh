...
VERSION_CURRENT=`curl -sI https://1p-installers.s3.amazonaws.com/agent/bin/linux/production/1p-agent  |grep x-amz-meta-version`
TIME=1

if [ "$VERSION_INSTALLED" != "$VERSION_CURRENT" ]; then
	echo "Start Update 1P Agent"
	if curl -sLO https://1p-installers.s3.amazonaws.com/agent/bin/linux/production/1p-agent	; then
		echo "Stop 1P Agent"
